...
As predicted, we see a larger value for $b_s$ in the conjunction condition (mean = `r mean(feat$b_stick)`) compared to the feature condition (mean = `r mean(conj$b_stick)`). This is in line with previous data.