...
```{r plot_model_post, fig.height=2.8, fig.width=8, eval=FALSE}
######################################################
# plot model
######################################################
...
plot_model_fixed(m, d,
                 cl = c("A", "B"),
                 gt = list(bA = qlogis(pA), 
                           bS = b_stick, 
                           sig_d = sig_d,
                           sig_theta = sig_theta))  

ggsave("../plots/sensiv_sim.pdf", width = 8, height = 2.8)
ggsave("../plots/sensiv_sim.png", width = 8, height = 2.8)
```
...
```{r, fig.height=2.8}
 m %>% recover_types(d$found) %>%
    spread_draws(bA[block], bS[block], phi_dis[block], phi_dir[block]) %>%
    mutate(block = as_factor(block)) %>%
    ungroup() -> post
  
levels(post$block) <- cond_labels 

post %>% pivot_longer(c(bA, bS, phi_dis, phi_dir), names_to = "param") %>%
