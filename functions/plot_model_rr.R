...
plot_model_spatial <- function(m, d) {
  ...
  m %>% 
    spread_draws( phi_dis[block], phi_dir[block]) %>%
    mutate(block = as_factor(block),
  ...
  m %>% spread_draws(u[block, person],) %>%
    mutate(param = block %% 4,
           ...
           param = fct_recode(param, phi_dir = "1", phi_dis = "2")) %>%
    rename(uz = "u") -> post_u
  
  full_join(post %>%
              pivot_longer(c(phi_dis, phi_dir), names_to = "param", values_to = "u"), 
            post_u, 
  ...
  # draw distance tuning figure for foxed effect
    pmap_dfr(select(post, block, phi_dis), 
             function(phi_dis, t, block) tibble(t=t, block = block,
                                                       q = exp(-phi_dis * t)),
             t = distances) %>%
      group_by(block, t) %>%
    ...
    pmap_dfr(select(post_u, block, phi_dis, person), 
             function(phi_dis, t, block, person) tibble(t=t, block = block, person = person,
                                                         q = exp(-phi_dis * t)),
             t = distances) %>%
